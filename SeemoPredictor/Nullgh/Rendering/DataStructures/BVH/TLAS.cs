...
namespace NullEngine.Rendering.DataStructures.BVH
{
    public class hTLAS
    {
        ...
        public void LoadMeshFromFile(Vec3 pos, Vec3 rot, string objFileName) 
        {
            //Dictionary<string, MaterialData> materials = LoadMaterialsFromFile(filename + ".mtl");

            string[] lines = File.ReadAllLines(objFileName + (objFileName.EndsWith(".obj") ? "" : ".obj"));
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (line.Length > 0 && line[0] != '#' && split.Length >= 2)
                {
                    switch (split[0])
                    {
                        case "v":
                            {
                                if (true /*mat != 2*/) //except glazing mesh
                                {
                        ...
                        case "usemtl":
                            {
                                ...
                                switch (split[1])
                                {
                                    ...
                                    case "Exterior":
                                        mat = 1;
                                        break;
                                    case "Glazing": // glazing shouldn't be intersected..hum,,
                                        mat = 2;
                                        break;
            ...
            AABB aabb = AABB.CreateFromVertices(verticies, pos);
        ...
        public void LoadMeshFromPrimitive(List<float> vertices, List<int> triangles, List<int> mats)
        {

            Vec3 pos = new Vec3(0, 0, 0);
            Vec3 rot = new Vec3(0, 0, 0);

            AABB aabb = AABB.CreateFromVertices(vertices, pos);
            dMesh mesh = renderDataManager.addGbufferForID(hMeshes.Count, aabb, pos, rot, triangles, vertices, new List<float>(), mats);
        ...
        public dTLAS GetDTLAS()
        {
            if(isDirty)
            {
                rebuildTLAS(); //this makes an error
