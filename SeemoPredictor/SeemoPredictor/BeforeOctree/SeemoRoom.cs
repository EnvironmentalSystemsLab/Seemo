...
namespace SeemoPredictor
{
    public class SeemoRoom
    {
        ...
        public List<Brep> Windows { get; set; } = new List<Brep>();
        ...
        public Mesh windowAndRoom { get; set; } = new Mesh();
        public List<Mesh> windowMeshs { get; set; } = new List<Mesh>();
        public Mesh windowMesh { get; set; } = new Mesh();
        ...
        public void ComputeWindowMesh()
        {
            ...
            Mesh windowMeshLocal = new Mesh();
            Mesh windowMeshLocal2 = new Mesh();
            List<Mesh> windowMeshsLocal = new List<Mesh>();
            ...
            foreach (Brep wd in Windows)
            {
                ...
                windowMeshLocal.Append(windowBrepToMeshs[0]);
                windowMeshLocal2.Append(windowBrepToMeshs[0]);
                windowMeshsLocal.Add(windowBrepToMeshs[0]);

            }


            windowMesh = windowMeshLocal;
            windowMeshs = windowMeshsLocal;

            windowThreshold = windowMesh.Faces.Count;

            windowAndRoom = windowMeshLocal2;
            windowAndRoom.Append(Room);

        }


        public ResultDataSet ComputeWindowRay(int pointIndex, int vectorIndex)
        {
            ...
            List<Vector3d> winRayVectors0 = new List<Vector3d>();
            ...
            for (int i = 0; i < sceneVectors.Count; i++)
            {
                ...
                if (dtr > 0)
                {
                    ...
                    {
                        winRayVectors0.Add(sceneVectors[i]);
                    }
                }
            }

            was = winRayVectors0.Count / (5288.02083158);
            ...
            for (int i = 0; i < winRayVectors0.Count; i++)
            {
                var dtr = Rhino.Geometry.Intersect.Intersection.MeshRay(zoneMesh, new Ray3d(vp, winRayVectors0[i]), out zoneIhit);
                if (dtr > 0)
                {
                    Point3d ipt = vp + winRayVectors0[i] * dtr;
                    zPts.Add(ipt);
            ...
            resultData1.winRayVectors = winRayVectors0;
            resultData1.Dir = vd;
